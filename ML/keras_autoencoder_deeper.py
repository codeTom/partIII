...
from tensorflow.keras.layers import UpSampling2D, Reshape, Input, GaussianNoise, BatchNormalization
...
def encoder(tensor, trainable=True, name_prefix="encoder/", denoising=False):
    if denoising:
        x = GaussianNoise(0.07)(tensor)
    else:
    ...
    x = BatchNormalization()(x)
    x = Conv2D(128, (3,3), padding='same', name=f"{name_prefix}c7", activation='relu', trainable=trainable)(x)
    x = Conv2D(128, (3,3), padding='same', name=f"{name_prefix}c8", activation='relu', trainable=trainable)(x)
    x = MaxPool2D((2, 2), padding='same', name=f"{name_prefix}max_p4")(x)
    x = BatchNormalization()(x)
...
def decoder(vector, trainable=True, name_prefix="decoder/"):
    x = Dense(6272, trainable=trainable, name=f"{name_prefix}fc1", activation='relu')(vector)
    x = Reshape((7,7,128), name=f"{name_prefix}reshape")(x)
    x = BatchNormalization()(x)
    x = UpSampling2D((2,2), name=f"{name_prefix}up0")(x)
    x = Conv2D(128, (3,3), activation='relu', padding='same', trainable=trainable, name=f'{name_prefix}c8')(x)
    x = Conv2D(128, (3,3), activation='relu', padding='same', trainable=trainable, name=f'{name_prefix}c9')(x)
    x = BatchNormalization()(x)
...
def model(denoise=False):
    ind=Input(shape=(image_size,image_size,3))
    latent=encoder(ind, denoising=denoise)
...
def get_cell_sequence(smooth=False):
    if smooth:
        files=[f"{data_dir}/rem_s.npz"]
    else:
        files=[f"{data_dir}/rem.npz"]
    #,f"{data_dir}/positive_s.npz",f"{data_dir}/negative_s.npz"] #change here for more npzfiles
...
def train(autoencoder, smooth):
    ds=get_cell_sequence(smooth)
    ims=ds[0][0]
    test_batch=ds[0][0]
    print("images loaded")
    ...
    print(sampls)
    tests=test_batch[np.random.choice(sampls,5)]
...
def saveTestImages(m, npz_file="test_data/0.npz", outfolder="test_img"):
    #load the test data
...
if __name__ == "__main__":
    ...
    train(m, True)
    runTest(True)
    copyfile('weights_best.hdf5', 'weights_best_smooth.hdf5')
    #tf.keras.backend.clear_session()
    #m=model(denoise=den)
    #train(m, False)
    #runTest(False)
    #copyfile('weights_best.hdf5', 'weights_best_sharp.hdf5')
